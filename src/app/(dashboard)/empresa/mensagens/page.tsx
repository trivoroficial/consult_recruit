...
import { Search, Send, MessageCircle, User, Building2, Clock } from 'lucide-react'
import { useState } from 'react'

const conversas = [
  { id: 1, nome: 'João Silva', tipo: 'candidato', ultima: 'Estou disponível para entrevista amanhã', data: '09/07/2026', lida: false },
  { id: 2, nome: 'Maria Santos', tipo: 'candidato', ultima: 'Obrigado pela oportunidade!', data: '08/07/2026', lida: true },
          ...
          <div className="grid grid-cols-3 h-[600px]">
                ...
                {conversas.map((conv) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${conv.tipo === 'candidato' ? 'bg-green-100' : 'bg-purple-100'}`}>
                        {conv.tipo === 'candidato' ? <User className="h-5 w-5 text-green-600" /> : <MessageCircle className="h-5 w-5 text-purple-600" />}
                      </div>
            ...
            <div className="col-span-2 flex flex-col">
              <div className="p-4 border-b flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${conversaSelecionada?.tipo === 'candidato' ? 'bg-green-100' : 'bg-purple-100'}`}>
                  {conversaSelecionada?.tipo === 'candidato' ? <User className="h-5 w-5 text-green-600" /> : <Building2 className="h-5 w-5 text-purple-600" />}
                </div>
                <div>
                  <p className="font-semibold">{conversaSelecionada?.nome}</p>
                  <p className="text-xs text-gray-500">{conversaSelecionada?.tipo === 'candidato' ? 'Candidato' : 'Suporte'}</p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-start">
                    <div className="bg-white p-3 rounded-lg shadow-sm max-w-[70%] border">
                      <p className="text-sm">Olá! Estou disponível para entrevista amanhã.</p>
                  ...
                  <div className="flex justify-end">
                    <div className="bg-purple-600 text-white p-3 rounded-lg max-w-[70%]">
                      <p className="text-sm">Perfeito! Confirmei sua entrevista para amanhã às 14h.</p>
