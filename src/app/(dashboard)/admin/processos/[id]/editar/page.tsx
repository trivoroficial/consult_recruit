...
export default function EditarProcesso() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setForm({
          ...
          candidatos: data.candidatos?.toString() || '',
          status: data.status || 'triagem',
          inicio: data.inicio ? data.inicio.split('T')[0] : '',
          previsao_fim: data.previsao_fim ? data.previsao_fim.split('T')[0] : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await atualizarProcesso(parseInt(id), {
        ...form,
        ...
        <div className="flex-1 ml-64 flex flex-col min-h-screen">
          <div className="flex-1 p-8 flex items-center justify-center">
            <div className="bg-white rounded-2xl shadow-sm border border-[#E8EAE0] p-12 text-center max-w-md w-full">
              ...
              <h2 className="text-2xl font-bold text-[#2D343A]">Processo atualizado com sucesso!</h2>
              <button
        ...
        <header className="bg-white border-b border-[#E8EAE0] px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#708090]">Atualize os dados do processo</p>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 p-8">
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-[#E8EAE0] p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-[#2D343A] mb-1.5">Nº de Candidatos</label>
                <input
                  type="number"
                  className="w-full px-4 py-3 border border-[#E8EAE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6B1A2A] transition"
              ...
              <div>
                <label className="block text-sm font-medium text-[#2D343A] mb-1.5">Previsão de Término</label>
                <input
                  type="date"
                  className="w-full px-4 py-3 border border-[#E8EAE0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6B1A2A] transition"
                  value={form.previsao_fim}
                  onChange={(e) => setForm({...form, previsao_fim: e.target.value})}
                />
              </div>
