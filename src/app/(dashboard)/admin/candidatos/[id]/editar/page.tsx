...
import { buscarCandidatoPorId, atualizarCandidato } from '@/actions/candidatos'
...
export default function EditarCandidato() {
  ...
  const carregarCandidato = async () => {
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'Erro ao carregar candidato')
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await atualizarCandidato(parseInt(id), form)
      if (result.success) {
        setSuccess(true)
        setTimeout(() => {
        ...
        setError(result.error || 'Erro ao atualizar candidato')
        setLoading(false)
      }
    } catch (err) {
        ...
        <div className="flex-1 ml-64 flex flex-col min-h-screen">
          <div className="flex-1 p-8 flex items-center justify-center">
            <div className="bg-white rounded-2xl shadow-sm border border-[#E8EAE0] p-12 text-center max-w-md w-full">
              ...
              <h2 className="text-2xl font-bold text-[#2D343A]">Candidato atualizado com sucesso!</h2>
              <button
        ...
        <header className="bg-white border-b border-[#E8EAE0] px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#708090]">Atualize os dados do candidato</p>
            </div>
          </div>
        </header>
