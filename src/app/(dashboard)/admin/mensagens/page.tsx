...
import { Search, Send, MessageCircle, Building2, User, Clock } from 'lucide-react'
import { useState } from 'react'

const conversas = [
  { id: 1, nome: 'Empresa XPTO', tipo: 'empresa', ultima: 'Precisamos de mais candidatos para a vaga', data: '09/07/2026', lida: false },
  { id: 2, nome: 'João Silva', tipo: 'candidato', ultima: 'Qual o prazo para resposta?', data: '08/07/2026', lida: true },
  { id: 3, nome: 'Indústria ABC', tipo: 'empresa', ultima: 'Contrato assinado com sucesso', data: '05/07/2026', lida: true },
]
          ...
          <div className="grid grid-cols-3 h-[600px]">
                ...
                {conversas.map((conv) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${conv.tipo === 'empresa' ? 'bg-blue-100' : 'bg-green-100'}`}>
                        {conv.tipo === 'empresa' ? <Building2 className="h-5 w-5 text-blue-600" /> : <User className="h-5 w-5 text-green-600" />}
                      </div>
            ...
            <div className="col-span-2 flex flex-col">
              <div className="p-4 border-b flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${conversaSelecionada?.tipo === 'empresa' ? 'bg-blue-100' : 'bg-green-100'}`}>
                  {conversaSelecionada?.tipo === 'empresa' ? <Building2 className="h-5 w-5 text-blue-600" /> : <User className="h-5 w-5 text-green-600" />}
                </div>
                <div>
                  <p className="font-semibold">{conversaSelecionada?.nome}</p>
                  <p className="text-xs text-gray-500">{conversaSelecionada?.tipo === 'empresa' ? 'Empresa' : 'Candidato'}</p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-start">
                    <div className="bg-white p-3 rounded-lg shadow-sm max-w-[70%] border">
                      <p className="text-sm">Precisamos de mais candidatos para a vaga de Analista.</p>
                  ...
                  <div className="flex justify-end">
                    <div className="bg-purple-600 text-white p-3 rounded-lg max-w-[70%]">
                      <p className="text-sm">Vamos priorizar essa vaga hoje mesmo.</p>
