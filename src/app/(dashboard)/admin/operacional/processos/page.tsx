...
import {
  ...
  CheckCircle, XCircle, Filter, Briefcase
} from 'lucide-react'
import { listarProcessosOperacionais, excluirProcessoOperacional } from '@/actions/operacional'

export default function AdminProcessosOperacionais() {
  ...
  const carregarProcessos = async () => {
    ...
    setError(null)
    try {
      const result = await listarProcessosOperacionais()
      if (result.success) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const result = await excluirProcessoOperacional(id)
      if (result.success) {
      ...
      <div className="flex-1 ml-64 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-[#E8EAE0] text-center">
              <p className="text-2xl font-bold text-green-600">{statusCounts.ativo}</p>
              <p className="text-xs text-[#708090]">Ativos</p>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-sm border border-[#E8EAE0] text-center">
              <p className="text-2xl font-bold text-yellow-600">{statusCounts.pausado}</p>
              <p className="text-xs text-[#708090]">Pausados</p>
            </div>
            <div className="bg-white p-4 rounded-xl shadow-sm border border-[#E8EAE0] text-center">
              <p className="text-2xl font-bold text-blue-600">{statusCounts.concluido}</p>
                    ...
                    {filtered.map((item) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-[#E8EAE0] hover:bg-[#F8F4E6] transition">
                          ...
                          <td className="py-3 px-4 text-[#708090]">{item.vagas || 0}</td>
                          <td className="py-3 px-4">
                            <div className="flex gap-2">
                              ...
                              <button 
                                onClick={() => router.push(`/admin/operacional/processos/${item.id}/editar`)}
