...
import { buscarEmpresaPorId, atualizarEmpresa } from '@/actions/empresas'
...
export default function EditarEmpresa() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setForm({
          ...
          cidade: data.cidade || '',
          estado: data.estado || '',
          funcionarios: data.funcionarios?.toString() || '',
        ...
        setError(result.error || 'Erro ao carregar empresa')
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await atualizarEmpresa(parseInt(id), form)
      if (result.success) {
        setSuccess(true)
        setTimeout(() => {
        ...
        <div className="flex-1 ml-64 flex flex-col min-h-screen">
          <div className="flex-1 p-8 flex items-center justify-center">
            <div className="bg-white rounded-2xl shadow-sm border border-[#E8EAE0] p-12 text-center max-w-md w-full">
              ...
              <h2 className="text-2xl font-bold text-[#2D343A]">Empresa atualizada com sucesso!</h2>
              <button
        ...
        <header className="bg-white border-b border-[#E8EAE0] px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-[#708090]">Atualize os dados da empresa</p>
            </div>
          </div>
        </header>
